...
export async function generateStyledImage(
  ...
  const prompt = [
    ...
    'The jewellery must remain the dominant, sharp commercial subject and fill at least 80% of the square (large product, not a postage stamp on empty silk).',
    'Edit from the isolated original layout. If the source is an OPEN V (chains to the top corners, earrings above, no clasp in frame), keep that open V. Do not close the necklace. Do not invent a clasp.',
    JEWELLERY_PRODUCT_LOCK_PROMPT,
    CATALOG_LAYOUT_LOCK_PROMPT,
...
export async function generateModelImage(
  ...
  const prompt = [
    ...
    'The jewellery is the focal commercial product and must be large enough to read the pendant and earring design.',
    ...
    JEWELLERY_PRODUCT_LOCK_PROMPT,
    'Keep both earrings anatomically wearable and faithful: same top stud shape, lower jhumka/dangler shape, stone placement, and dangling drop as the reference.',
    'Do not invent a different necklace or earrings. Do not add competing jewellery. Do not change the pendant design, earring design, stone colours, or clasp.',
    'TIGHT LISTING CROP: frame from the chin/lower lip to just below the pendant drop. The necklace and both earrings must be large, sharp, and the dominant commercial subject — readable catalogue scale, not a distant wide torso.',
    'Do not show a wide shoulders-to-waist crop or empty blouse. Fill the square with neckline, collarbone, pendant, and earrings. Soft premium lighting and realistic skin tones.',
    'The complete pendant, dangling drop, and both earrings must stay fully visible with a small breathing margin. Do not crop jewellery edges.',
    'Show the necklace path on the neck down to the complete pendant, and show both earrings on the ears.',
    params.customPrompt ? `Additional user direction: ${params.customPrompt}` : '',
...
export async function generateWhiteProductPresentationImage(
  ...
  const basePrompt = [
    ...
    CATALOG_LAYOUT_LOCK_PROMPT,
    '- Preserve visible top closures such as cylindrical barrel clasps, tube clasps, hooks, knots or connector pieces ONLY when they appear in the source. Never invent a clasp.',
